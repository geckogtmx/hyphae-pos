...
export const calculatePackagingFallback = (orderPayload: OrderPayload): CalculationResult => {
    ...
    orderPayload.items.forEach(item => {
        ...
        const hasOnSide = safeModifiers.some(mod => mod && typeof mod === 'string' && mod.toLowerCase().includes('side'));
    ...
    if ((orderPayload.serviceType === 'Takeout' || orderPayload.serviceType === 'Delivery') && totalItems > 0) {
    ...
    const napkinCount = GLOBAL_ITEMS.NAPKIN_BASE + 
