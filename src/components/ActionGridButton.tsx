...
const ActionGridButton: React.FC<ActionGridButtonProps> = ({
  ...
  const onLongPress = () => { if (onVariationChange) setShowModal(true); };
  const handleVariationSelect = (v: ModifierVariation) => {
    if (onVariationChange) onVariationChange(v);
    setShowModal(false);
  };

  const { onMouseDown, onMouseUp, onMouseLeave, onTouchStart, onTouchEnd } = useLongPress(onLongPress, onClick, { delay: 400 });

  const isVariation = variation !== 'Normal';
  const isLime = accentColor === 'lime-400';
  
  const borderColor = selected 
    ? (isLime ? 'border-lime-500 dark:border-lime-400' : `border-${accentColor}`) 
    : 'border-zinc-200 dark:border-zinc-800';
    
  const textColor = selected 
    ? (isLime ? 'text-lime-700 dark:text-lime-400' : `text-${accentColor}`) 
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <button onMouseDown={onMouseDown} onMouseUp={onMouseUp} onMouseLeave={onMouseLeave} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} className={`w-full h-full p-4 flex flex-col justify-between items-start rounded-xl border-2 transition-all active:scale-[0.98] relative overflow-hidden ${bgColor} ${borderColor} ${shadowClass}`}>
        {selected && (
        ...
        <div className="w-full text-left pr-6">
          ...
          {isVariation && (
            <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-md mt-1 inline-block ${variation === 'No' ? 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400' : variation === 'Side' ? 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400' : 'bg-lime-100 dark:bg-lime-900 text-lime-600 dark:text-lime-400'}`}>{variation}</span>
          )}
        </div>
        
        {price !== undefined && price > 0 && <span className={`font-mono text-sm mt-2 ${selected ? 'text-zinc-500 dark:text-zinc-400' : 'text-zinc-400 dark:text-zinc-500'}`}>+${price.toFixed(2)}</span>}
        {!showModal && onVariationChange && <div className={`absolute bottom-2 right-2 w-1.5 h-1.5 rounded-full ${selected ? 'bg-lime-500/50 dark:bg-lime-400/50' : 'bg-zinc-300 dark:bg-zinc-700'} opacity-50`} />}
      </button>
    </div>
  );
};
