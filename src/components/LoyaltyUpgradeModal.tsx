      ...
      <div className="relative bg-zinc-900 border border-zinc-800 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-gradient-to-r from-lime-900/50 to-zinc-900 p-6 flex items-center justify-between border-b border-lime-500/30">
            <div className="flex items-center space-x-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold text-white uppercase italic tracking-wider">Tier Upgrade!</h2>
                    <p className="text-lime-400 font-mono text-sm">Customer has reached the next level.</p>
                </div>
            </div>
        </div>
        ...
        <div className="p-8 flex gap-8">
            ...
            <div className="flex-1 space-y-6">
                <div className="space-y-2">
                    <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Customer</span>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex-1 p-4 bg-zinc-950 rounded-xl border border-zinc-800 opacity-60">
                        ...
                        <span className={`text-lg font-bold text-${oldTierData?.color || 'zinc-400'}`}>{oldTierData?.name}</span>
                    </div>
                    ...
                    <div className="flex-1 p-4 bg-zinc-950 rounded-xl border border-lime-500/50 relative overflow-hidden">
                        ...
                        <span className="text-xs text-lime-400 uppercase block mb-1 font-bold">New Tier</span>
                        <span className={`text-xl font-bold text-${newTierData?.color || 'white'} uppercase`}>{newTierData?.name}</span>
                    </div>
                </div>
                ...
                <div className="p-4 bg-red-900/10 border border-red-900/30 rounded-xl flex items-start space-x-3">
                    ...
                    <div className="text-sm text-red-200">
                        <strong className="block text-red-400 uppercase text-xs mb-1">Action Required</strong>
                        Recover card <span className="font-mono font-bold">{profile.activeCard?.code || 'NO_CARD'}</span> from customer and destroy/recycle it.
                    </div>
                </div>
            </div>
            ...
            <div className="flex-[1.5] bg-zinc-950 p-6 rounded-2xl border border-zinc-800 flex flex-col">
                ...
                <div className="grid grid-cols-5 gap-1 mb-2">
                    {KEYBOARD_NUMBERS.map(k => (
                        <button key={k} onClick={() => handleInput(k)} className="h-10 bg-zinc-800 rounded font-mono font-bold text-zinc-300 hover:bg-zinc-700">{k}</button>
                    ))}
                </div>
                <div className="grid grid-cols-4 gap-1 mb-4">
                    {KEYBOARD_LETTERS.map(k => (
                        <button key={k} onClick={() => handleInput(k)} className="h-10 bg-zinc-800 rounded font-mono font-bold text-zinc-400 hover:bg-zinc-700">{k}</button>
                    ))}
                </div>

                <div className="flex gap-2 mt-auto">
                     <button onClick={handleBackspace} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 rounded-lg py-3 font-bold uppercase text-xs">Correction</button>
                     <button 
