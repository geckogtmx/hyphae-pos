                                ...
                                {!isCollapsed ? (
                                    <>
                                        <div>
                                            ...
                                            <div className="text-[10px] text-zinc-500 font-bold uppercase">{order.table}</div>
                                        </div>
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-start pt-4 space-y-2">
                                        <span className="text-white font-mono font-bold text-lg [writing-mode:vertical-lr] rotate-180">#{order.id}</span>
                                        <div className={`w-2 h-2 rounded-full ${isReady ? 'bg-lime-500' : 'bg-orange-500'}`} />
                                    </div>
                                )}
                            </div>
                                        ...
                                        {bundles.map((bundle: any, idx: number) => (
                                            <div key={`${bundle.uniqueId}-${idx}`} className="bg-zinc-950 border border-zinc-800 rounded-lg p-3 flex flex-col">
                                                <div className="flex justify-between items-start mb-2 border-b border-zinc-800 pb-2">
                                                    ...
                                                    <span className="bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded text-xs font-mono font-bold">x{bundle.qty}</span>
                                                </div>
                                                <div className="space-y-2 mb-2 flex-1">
                                                    {bundle.mods.map((mod: string, i: number) => (
                                                        <div key={i} className="text-xs px-2 py-1 rounded bg-blue-900/20 text-blue-300 border border-blue-900/50 inline-block mr-1 mb-1 font-bold uppercase">
                                                            {mod}
                                                        </div>
                                                    ))}
                                                </div>
                                                        ...
                                                        {bundle.sides.map((side: any, i: number) => (
                                                            <div key={i} className="text-xs text-zinc-400 bg-zinc-900 px-2 py-1 rounded border border-zinc-800 mb-1 flex items-center flex-wrap gap-2">
                                                                ...
                                                                {side.modifiers.map((mod: string, j: number) => (
                                                                    <span key={j} className="text-[10px] px-1.5 py-0.5 bg-zinc-800 text-zinc-500 rounded border border-zinc-700">
                                                                        {mod}
                                                                    </span>
                                                                ))}
                                                            </div>
                                    ...
                                    <div className="shrink-0 p-3 bg-zinc-900/50 border-t border-zinc-800">
                                        <div className="flex flex-wrap gap-2">
                                            {Object.entries(orderPkg).map(([sku, count]) => (
                                                <span key={sku} className="text-[9px] font-mono font-bold uppercase px-2 py-1 bg-zinc-800 text-zinc-400 rounded border border-zinc-700 flex items-center">
                                                    <span className="text-lime-500 mr-1">{count}x</span> {sku.replace('SKU_', '')}
                                                </span>
                                            ))}
                                        </div>
                                    </div>
