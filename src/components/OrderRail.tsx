...
import { Wand2, UtensilsCrossed, ChevronUp, ChevronDown, Bell, CheckCircle, ChefHat, Edit } from 'lucide-react';
...
const OrderRail: React.FC<OrderRailProps> = ({ onLayoutChange: _onLayoutChange }) => {
    ...
    const queueOrders = useMemo(() =>
        activeOrders.filter(o => o.status === 'Pending').sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0))
        , [activeOrders]);

    const cookingOrders = useMemo(() =>
        activeOrders.filter(o => o.status === 'Kitchen').sort((a, b) => (a.cookingStartedAt || 0) - (b.cookingStartedAt || 0))
        , [activeOrders]);

    const readyOrders = useMemo(() =>
        activeOrders.filter(o => o.status === 'Ready').sort((a, b) => (a.readyAt || 0) - (b.readyAt || 0))
        , [activeOrders]);

    const assemblyOrders = useMemo(() => [...cookingOrders, ...readyOrders], [cookingOrders, readyOrders]);
    const totalActiveOrders = readyOrders.length + cookingOrders.length + queueOrders.length;

    const handleStatusUpdate = useCallback((order: SavedOrder, newStatus: string) => {
    ...
    const finalizeCompletion = useCallback((order: SavedOrder, tipAmount: number) => {
        dispatch({
            type: 'UPDATE_ORDER',
            payload: {
                ...order,
    ...
    const handleEditOrder = useCallback((order: SavedOrder) => {
    ...
    const handleManualReorder = useCallback((orderId: string, direction: 'UP' | 'DOWN') => {
        ...
        if (targetIndex >= 0 && targetIndex < newQueue.length) {
            ...
            const documentExtended = document as Document & { startViewTransition?: (cb: () => void) => void };
                ...
                <div key={i} className="text-xs">
                    ...
                    <div className="pl-3 space-y-0.5 mt-0.5">
                        {(item.selectedModifiers || []).map((mod, j) => (
                            <div key={j} className={`text-[10px] uppercase ${mod.variation === 'No' ? 'text-red-500' : mod.variation === 'Extra' ? 'text-lime-600 dark:text-lime-400' : 'text-zinc-500'}`}>
                                {mod.variation !== 'Normal' ? `${mod.variation} ` : ''}{mod.name}
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <>
                        ...
                        <div className="p-2 space-y-2 pb-4">
                            {readyOrders.map(order => {
                                ...
                                return (
                                    <div key={order.id} className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 flex flex-col hover:border-lime-500/50 transition-all shadow-sm">
                                        <div className="flex justify-between items-start mb-2 cursor-pointer" onClick={(e) => toggleOrderExpand(order.id, e)}>
                                            <div className="flex items-center">
                                                <div className="mr-2 text-zinc-400">{isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
                                                <div>
                                                    <span className="font-mono font-bold text-lg text-zinc-900 dark:text-white">#{order.id}</span>
                                                    <span className="text-[10px] font-bold text-zinc-500 uppercase block">{order.table}</span>
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end">
                                                <LiveTimer startTime={order.readyAt} className="font-mono font-bold text-sm text-lime-600" />
                                                <div className="flex items-center gap-1 mt-1">
                                                    {order.isLoyalty && <span className="text-[9px] font-bold bg-yellow-400 text-zinc-900 px-1.5 py-0.5 rounded uppercase">{getVipLabel(order.loyaltySnapshot?.tierName)}</span>}
                                                    <span className="text-[9px] uppercase text-zinc-400">READY</span>
                                                </div>
                                            </div>
                                        </div>
                                        {isExpanded && renderOrderItems(order)}
                                        <button onClick={() => setCompletionOrder(order)} className="w-full py-3 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-lg text-xs font-bold uppercase flex items-center justify-center"><CheckCircle size={16} className="mr-2" /> PACK & CLOSE</button>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <>
                        ...
                        <div className="p-2 space-y-2 pb-4">
                            {cookingOrders.map(order => {
                                const isExpanded = expandedOrderIds.has(order.id);
                                return (
                                    <div key={order.id} className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 flex flex-col hover:border-orange-500/50 transition-all shadow-sm">
                                        <div className="flex justify-between items-start mb-2 cursor-pointer" onClick={(e) => toggleOrderExpand(order.id, e)}>
                                            <div className="flex items-center">
                                                <div className="mr-2 text-zinc-400">{isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
                                                <div>
                                            ...
                                            <div className="flex flex-col items-end">
                                                <LiveTimer startTime={order.cookingStartedAt} className="font-mono font-bold text-sm text-orange-600" />
                                                <div className="flex items-center gap-1 mt-1">
                                                    {order.isLoyalty && <span className="text-[9px] font-bold bg-yellow-400 text-zinc-900 px-1.5 py-0.5 rounded uppercase">{getVipLabel(order.loyaltySnapshot?.tierName)}</span>}
                                        ...
                                        {isExpanded && renderOrderItems(order)}
                                        <button onClick={() => handleStatusUpdate(order, 'Ready')} className="w-full py-3 bg-lime-500 hover:bg-lime-400 rounded-lg text-xs font-bold uppercase text-zinc-950 flex items-center justify-center shadow-lg shadow-lime-500/20"><Bell size={16} className="mr-2" /> ORDER READY</button>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <>
                        ...
                        <div className="p-2 space-y-2 pb-20">
                            {queueOrders.map(order => {
                                const isExpanded = expandedOrderIds.has(order.id);
                                return (
                                    <div key={order.id} className={`bg-zinc-50 dark:bg-zinc-900 rounded-xl p-2 flex flex-col transition-all relative group shadow-sm ${order.isLoyalty ? 'border border-yellow-400 dark:border-yellow-600 shadow-[0_0_10px_rgba(250,204,21,0.1)]' : 'border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
                                        <div className="flex h-full">
                                            <div className="w-12 flex flex-col items-center justify-center border-r border-zinc-200 dark:border-zinc-800 pr-2 mr-2 gap-1 py-2">
                                                <button onClick={() => handleManualReorder(order.id, 'UP')} className="flex-1 w-full flex items-center justify-center text-zinc-400 hover:text-lime-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded"><ChevronUp size={20} /></button>
                                                <button onClick={() => handleManualReorder(order.id, 'DOWN')} className="flex-1 w-full flex items-center justify-center text-zinc-400 hover:text-lime-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded"><ChevronDown size={20} /></button>
                                            </div>
                                            ...
                                            <div className="flex-1 py-1">
                                                <div className="flex justify-between items-start mb-2 cursor-pointer" onClick={(e) => toggleOrderExpand(order.id, e)}>
                                                    <div className="flex items-center">
                                                        <div className="mr-2 text-zinc-400">{isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
                                                        <div>
                                                            <div className="font-mono font-bold text-lg text-zinc-900 dark:text-white">#{order.id}</div>
                                                            <div className="text-[10px] font-bold text-zinc-500 uppercase">{order.table}</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right flex flex-col items-end">
                                                        <span className="font-mono font-bold text-sm text-zinc-900 dark:text-white">${order.total.toFixed(2)}</span>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            ...
                                                            <span className="text-[9px] font-bold bg-zinc-200 dark:bg-zinc-800 text-zinc-500 px-1.5 py-0.5 rounded uppercase">PENDING</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex gap-2 mt-3">
                                                    <button onClick={() => handleEditOrder(order)} className="flex-1 py-2 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 rounded-lg text-xs font-bold uppercase text-zinc-600 dark:text-zinc-400 flex items-center justify-center"><Edit size={14} className="mr-1" /> Edit</button>
                                                    <button onClick={() => handleStatusUpdate(order, 'Kitchen')} className="flex-[2] py-2 bg-orange-500 hover:bg-orange-400 rounded-lg text-xs font-bold uppercase text-white flex items-center justify-center"><ChefHat size={14} className="mr-1" /> Fire</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
