...
import { X, Plus, ChevronRight, Trash2, GripVertical, CheckCircle, Save, Layers, Link, Utensils, ChevronLeft, ChefHat, Check } from 'lucide-react';
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onClose, activeConceptId }) => {
    ...
    const handleCreateSequence = () => {
        ...
        const newSeq: Product = { id: generateId('seq'), name: 'New Sequence', price: 0, categoryId: defaultCategory, requiresMods: true, modifierGroups: [] };
    ...
    const handleAddStep = () => {
        if (!activeSequence) return;
        const newStep: ModifierGroup = { id: generateId('step'), name: 'New Modifier Step', required: true, multiSelect: false, options: [] };
    ...
    const handleInsertStep = (visualIndex: number) => {
        ...
        const newStep: ModifierGroup = { id: generateId('step'), name: 'New Step', required: true, multiSelect: false, options: [] };
    ...
    const handleAddOption = () => {
        ...
        updateCurrentGroup({ options: [...currentGroup.options, { id: generateId('opt'), name: 'New Item', price: 0 }] });
    };
                            ...
                            <div className="flex-1 p-8 overflow-y-auto pb-48">
                                <div className="flex items-center justify-between mb-8 border-b border-zinc-200 dark:border-zinc-900 pb-4 pl-4">
                                    <div className="flex items-center">
                                        {isActiveStepRoot ? <span className="bg-lime-100 dark:bg-lime-400/10 text-lime-700 dark:text-lime-400 px-3 py-1 rounded text-sm font-bold uppercase mr-3">MAIN ITEM (ROOT)</span> : (activeVisualStep as { type?: string }).type === 'SUB_ITEM' ? <span className="bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-3 py-1 rounded text-sm font-bold uppercase mr-3">ADDITIONAL ITEM</span> : <span className="bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 px-3 py-1 rounded text-sm font-bold uppercase mr-3">MODIFIER STEP</span>}
