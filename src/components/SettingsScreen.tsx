...
import { X, Plus, ChevronRight, Trash2, GripVertical, CheckCircle, Circle, Save, Layers, ArrowRight, Link, Box, Utensils, ChevronLeft, Check, ChefHat } from 'lucide-react';
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onClose, activeConceptId }) => {
  const { getProductsByConcept, concepts, categories, saveBatchProducts, products } = useMenuData(); 
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [saveStatus, setSaveStatus] = useState<'IDLE' | 'SAVING' | 'SAVED'>('IDLE');
  const [expandedOptionId, setExpandedOptionId] = useState<string | null>(null);
  const [sequences, setSequences] = useState<Product[]>(() => getProductsByConcept(activeConceptId));
  const [selectedSequenceId, setSelectedSequenceId] = useState<string | null>(sequences.length > 0 ? sequences[0].id : null);
  ...
  const visualSteps = useMemo(() => {
      ...
      const rootStep = { id: 'ROOT', name: activeSequence.name, type: 'ROOT_ITEM', price: activeSequence.price, options: [], required: true };
      const groupSteps = (activeSequence.modifierGroups || []).map(g => ({ ...g, type: g.variant === 'sub_item' ? 'SUB_ITEM' : 'MODIFIER' }));
  ...
  const handleSelectSequence = (id: string) => { setSelectedSequenceId(id); setSelectedStepId('ROOT'); };
  const handleCreateSequence = () => {
    const defaultCategory = categories.find(c => c.conceptId === activeConceptId)?.id || 'burgers';
    const newSeq: Product = { id: `seq_${Date.now()}`, name: 'New Sequence', price: 0, categoryId: defaultCategory, requiresMods: true, modifierGroups: [] };
    setSequences([...sequences, newSeq]);
    setSelectedSequenceId(newSeq.id);
    setSelectedStepId('ROOT');
  };
  const updateSequence = (updated: Product) => setSequences(prev => prev.map(s => s.id === updated.id ? updated : s));

  const handleAddStep = () => {
    if (!activeSequence) return;
    const newStep: ModifierGroup = { id: `step_${Date.now()}`, name: 'New Modifier Step', required: true, multiSelect: false, options: [] };
    updateSequence({ ...activeSequence, modifierGroups: [...(activeSequence.modifierGroups || []), newStep] });
  ...
  const handleInsertStep = (visualIndex: number) => {
    ...
    const newStep: ModifierGroup = { id: `step_${Date.now()}`, name: 'New Step', required: true, multiSelect: false, options: [] };
    const currentGroups = activeSequence.modifierGroups || [];
    updateSequence({ ...activeSequence, modifierGroups: [...currentGroups.slice(0, arrayIndex), newStep, ...currentGroups.slice(arrayIndex)] });
    setSelectedStepId(newStep.id);
  };

  const updateCurrentGroup = (updates: Partial<ModifierGroup>) => {
      if (!activeSequence || isActiveStepRoot) return;
      updateSequence({ ...activeSequence, modifierGroups: (activeSequence.modifierGroups || []).map(group => group.id === selectedStepId ? { ...group, ...updates } : group) });
  };
  ...
  const handleAddOption = () => {
      ...
      updateCurrentGroup({ options: [...currentGroup.options, { id: `opt_${Date.now()}`, name: 'New Item', price: 0 }] });
  };
  ...
  const updateOption = (optId: string, updates: Partial<ModifierOption>) => {
      ...
      const currentGroup = activeSequence.modifierGroups?.find(g => g.id === selectedStepId);
      if (!currentGroup) return;
      updateCurrentGroup({ options: currentGroup.options.map(opt => opt.id === optId ? { ...opt, ...updates } : opt) });
  };
    ...
    <div className="fixed inset-x-0 bottom-0 top-14 bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-950 z-[45] flex flex-col animate-in slide-in-from-bottom duration-300 transition-colors">
      ...
      <div className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 flex items-center justify-between px-6 shrink-0 z-20">
         <div className="flex items-center space-x-4">
            ...
            <div>
                <h1 className="text-xl font-bold text-zinc-900 dark:text-white tracking-tight">MENU BUILDER</h1>
                <div className="flex items-center space-x-2 text-xs font-mono uppercase">
                    ...
                    <span className={`font-bold text-${currentConcept?.color.split('-')[0]}-600 dark:text-${currentConcept?.color.split('-')[0]}-400`}>
                        {currentConcept?.name}
                    </span>
                </div>
            </div>
         ...
         <div className="flex items-center space-x-4">
             <button onClick={handleSave} disabled={saveStatus !== 'IDLE'} className={`flex items-center px-4 py-2 rounded-lg font-bold text-sm transition-all min-w-[140px] justify-center ${saveStatus === 'SAVED' ? 'bg-zinc-100 dark:bg-zinc-100 text-zinc-900 dark:text-zinc-950 shadow-[0_0_15px_rgba(255,255,255,0.4)]' : saveStatus === 'SAVING' ? 'bg-lime-500/50 text-zinc-900 cursor-wait' : 'bg-lime-500 hover:bg-lime-400 text-zinc-950 shadow-[0_0_15px_rgba(132,204,22,0.3)]'}`}>
                 {saveStatus === 'SAVED' ? <><Check size={18} strokeWidth={3} className="mr-2 animate-in zoom-in duration-200" />SAVED!</> : saveStatus === 'SAVING' ? <><Save size={18} className="mr-2 animate-pulse" />SAVING...</> : <><Save size={18} className="mr-2" />SAVE CHANGES</>}
             </button>
             <button onClick={onClose} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full text-zinc-500 hover:text-black dark:hover:text-white transition-colors"><X size={24} /></button>
         </div>
      </div>
              ...
              {activeSequence ? (
                <>
                  <div className="flex-1 p-8 overflow-y-auto pb-48"> 
                    <div className="flex items-center justify-between mb-8 border-b border-zinc-200 dark:border-zinc-900 pb-4 pl-4">
                        <div className="flex items-center">
                             {isActiveStepRoot ? <span className="bg-lime-100 dark:bg-lime-400/10 text-lime-700 dark:text-lime-400 px-3 py-1 rounded text-sm font-bold uppercase mr-3">MAIN ITEM (ROOT)</span> : (activeVisualStep as any).type === 'SUB_ITEM' ? <span className="bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-3 py-1 rounded text-sm font-bold uppercase mr-3">ADDITIONAL ITEM</span> : <span className="bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 px-3 py-1 rounded text-sm font-bold uppercase mr-3">MODIFIER STEP</span>}
                             <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">{activeVisualStep.name}</h2>
                        </div>
                    </div>
                            ...
                            {isActiveStepRoot && (
                                <>
                                    ...
                                    <div className="p-4 bg-lime-50 dark:bg-lime-900/10 rounded-xl border border-lime-200 dark:border-lime-900/30">
                                        <h4 className="text-xs font-bold text-lime-700 dark:text-lime-400 uppercase tracking-widest mb-3 flex items-center"><ChefHat size={12} className="mr-1"/> Kitchen Prep Settings</h4>
                            ...
                            {!isActiveStepRoot && (
                             ...
                             {!isActiveStepRoot && !(activeVisualStep as ModifierGroup).variant && (
                                 <div className="flex items-start p-3 bg-zinc-100 dark:bg-zinc-800/30 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700 text-xs text-zinc-500 dark:text-zinc-400">
                                     <Link size={14} className="mt-0.5 mr-2 shrink-0 text-lime-600 dark:text-lime-500" />
                                     <div><span className="font-bold text-zinc-700 dark:text-zinc-300 block mb-1">Auto-Linked Dependency</span>Depending on: <span className="text-black dark:text-white font-mono">{getParentItemName(visualSteps.findIndex(s => s.id === selectedStepId))}</span></div>
                                 </div>
                             )}
                        </div>
                                ...
                                <div className="bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
                                    ...
                                    <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                                        {((activeVisualStep as ModifierGroup).options || []).map((opt) => (
                                            <div key={opt.id} className="flex flex-col gap-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-3 rounded-lg group hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-auto bg-zinc-100 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex flex-col z-10 shadow-lg pb-[env(safe-area-inset-bottom)]">
                      ...
                      <div ref={timelineRef} className="flex-1 overflow-x-auto p-4 flex items-center space-x-4 h-44">
                                  ...
                                  <div key={step.id} className="flex items-center">
                                      ...
                                      <button onClick={() => setSelectedStepId(step.id)} className={`w-44 h-28 rounded-xl border-2 flex flex-col p-3 transition-all relative overflow-visible text-left ${isActive ? 'bg-zinc-50 dark:bg-zinc-800 border-lime-500 shadow-md scale-105 z-10' : isRoot ? 'bg-lime-50 dark:bg-lime-900/10 border-lime-500/30' : isSubItem ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-500/50' : 'bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 hover:border-zinc-400'}`}>
                                          <div className="flex items-center gap-1 mb-2">
                                              ...
                                              {isModifier && <span className="text-[9px] font-bold bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 px-1.5 rounded">MOD</span>}
                                              <span className="text-[9px] font-mono text-zinc-500 ml-auto">STEP {idx + 1}</span>
                                          </div>
                                          <span className={`font-bold text-sm leading-tight truncate w-full mb-1 ${isActive ? 'text-black dark:text-white' : 'text-zinc-700 dark:text-zinc-300'}`}>{step.name}</span>
                                          {isModifier && <div className="mt-auto flex items-center text-[9px] text-zinc-500 truncate w-full"><Link size={10} className="mr-1 text-zinc-400" /><span className="truncate">for {parentName}</span></div>}
                                          {!isRoot && <div className={`mt-auto text-[9px] ${isModifier ? 'text-right' : ''}`}><span className="text-zinc-400">{(step as ModifierGroup).options?.length || 0} Opts</span></div>}
                                      </button>
                                  </div>
                              );
                          })}
                          <div className="flex items-center ml-4">
                              ...
                              <button onClick={handleAddStep} className="w-24 h-24 rounded-xl border-2 border-dashed border-zinc-300 dark:border-zinc-800 hover:border-lime-500/50 hover:bg-zinc-50 dark:hover:bg-zinc-900 flex flex-col items-center justify-center text-zinc-400 dark:text-zinc-600 hover:text-lime-600 dark:hover:text-lime-400 transition-all"><Plus size={24} className="mb-2" /><span className="text-[10px] uppercase font-bold">End</span></button>
                          </div>
                      </div>
                  </div>
                </>
              ) : (
                  <div className="flex-1 flex items-center justify-center text-zinc-400 dark:text-zinc-600">{sequences.length > 0 ? 'Select a sequence to edit' : 'No sequences available. Create one to get started.'}</div>
              )}
          </div>
      </div>
    </div>
