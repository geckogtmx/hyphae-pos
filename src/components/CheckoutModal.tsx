...
import { CreditCard, Banknote, ArrowRightLeft, Split, Check, X, DollarSign, List, Edit, Hash, RefreshCcw, Delete, PencilLine, Trash2, Plus, Coins } from 'lucide-react';
...
interface CheckoutModalProps {
  ...
  total: number;
  items: OrderItem[];
  onConfirmPayment: (method: PaymentMethod, amount: number, isFull: boolean, confirmationNumber?: string, tenderedAmount?: number, tipAmount?: number) => void;
...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, subtotal, tax, total, items, onConfirmPayment, amountAlreadyPaid = 0, onEditItem, onRemoveItem }) => {
  ...
  const groupedItems = useMemo(() => {
    const groups: Record<string, { key: string; mainItem: OrderItem; items: OrderItem[]; total: number }> = {};
  ...
  const renderOrderReview = () => (
    <div className="flex flex-col h-full animate-in slide-in-from-right duration-300">
                    ...
                    <div key={`${group.key}-${idx}`} className="bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 flex flex-col shadow-sm relative group hover:border-lime-500/50 transition-all h-full min-h-[180px]">
                        <div className="flex justify-between items-start mb-3 border-b border-zinc-100 dark:border-zinc-800/50 pb-2">
                            <div className="flex items-center min-w-0 pr-2">
                                ...
                                <span className="font-bold text-lg text-zinc-900 dark:text-zinc-100 leading-tight truncate" title={item.name}>{item.name}</span>
                            </div>
                            <span className="font-mono font-bold text-lg text-zinc-700 dark:text-zinc-300">${group.total.toFixed(2)}</span>
                        </div>

                        <div className="flex-1 content-start flex flex-wrap gap-1.5 mb-4 align-content-start">
                            {item.selectedModifiers?.filter(m => {
                                ...
                                if (group.dependency) {
                                    const parentGroup = item.modifierGroups?.find(pg => pg.id === group.dependency?.groupId);
                                    if (parentGroup?.variant === 'sub_item') return false;
                                }
                                return true;
                            }).map(mod => (
                                <span key={`${mod.groupId}-${mod.optionId}`} className="text-xs px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded text-zinc-600 dark:text-zinc-300 font-medium border border-zinc-200 dark:border-zinc-700">
                                    {mod.variation !== 'Normal' && <span className={`mr-1 font-bold ${mod.variation === 'No' ? 'text-red-500 dark:text-red-400' : 'text-blue-500 dark:text-blue-400'}`}>{mod.variation}</span>}{mod.name}
                                </span>
                            ))}

                            {item.selectedModifiers?.filter(m => item.modifierGroups?.find(g => g.id === m.groupId)?.variant === 'sub_item').map(subItem => (
                                <div key={subItem.optionId} className="flex items-center bg-zinc-50 dark:bg-zinc-900/50 rounded px-2 py-1 border border-zinc-200 dark:border-zinc-800 w-full mt-1">
                        ...
                        <div className="mt-auto pt-3 border-t border-zinc-100 dark:border-zinc-800 flex gap-2">
                            {onEditItem && (
                                <button onClick={() => onEditItem(item)} className="flex-1 py-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 rounded-lg text-xs font-bold uppercase flex items-center justify-center transition-colors">
                                    <PencilLine size={14} className="mr-1"/> Edit {count > 1 ? 'One' : ''}
                                </button>
                            )}
                            {onRemoveItem && (
                                <button onClick={() => group.items.forEach(i => onRemoveItem(i.uniqueId))} className="w-10 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 hover:bg-red-100 dark:hover:bg-red-900/30 text-zinc-400 hover:text-red-500 dark:hover:text-red-400 rounded-lg transition-colors">
                                    <Trash2 size={16} />
                                </button>
                            )}
                        </div>
        ...
        <div className="shrink-0 bg-zinc-50 dark:bg-zinc-950/80 border-t border-zinc-200 dark:border-zinc-800 px-6 py-4 flex items-center justify-between z-20 backdrop-blur-sm shadow-[0_-5px_25px_rgba(0,0,0,0.2)]">
             <div className="flex items-center gap-6 md:gap-8">
                 <div className="flex flex-col">
                     <span className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Subtotal</span>
                     <span className="font-mono text-3xl font-bold text-zinc-700 dark:text-zinc-300">${subtotal.toFixed(2)}</span>
                 </div>
                 ...
                 <div className="flex flex-col">
                     ...
                     <span className="font-mono text-3xl font-bold text-zinc-700 dark:text-zinc-300">${tax.toFixed(2)}</span>
                 </div>
                 {amountAlreadyPaid > 0 && (
                     <>
                         <div className="w-px h-12 bg-zinc-200 dark:bg-zinc-800"></div>
                         <div className="flex flex-col">
                             <span className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Paid</span>
                             <span className="font-mono text-3xl font-bold text-zinc-700 dark:text-zinc-300">-${amountAlreadyPaid.toFixed(2)}</span>
                         </div>
                     </>
                 )}
             </div>
             ...
             <div className="flex items-center gap-4 bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 px-8 py-4 rounded-xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
                 ...
                 <span className={`font-mono text-5xl font-bold ${isRefund ? 'text-red-500 dark:text-red-400' : 'text-lime-600 dark:text-lime-400'}`}>
        ...
        <div className="flex space-x-6 shrink-0 p-4 bg-zinc-50 dark:bg-zinc-950 pb-[calc(1rem+env(safe-area-inset-bottom))]">
            <button onClick={onClose} className="flex-1 h-20 bg-zinc-200 dark:bg-zinc-800 rounded-2xl font-bold uppercase text-zinc-500 dark:text-zinc-400 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors flex items-center justify-center space-x-3 text-lg"><Edit size={24} /><span>Modify Order</span></button>
            <button onClick={() => { if (isZeroBalance) handleZeroBalanceConfirm(); else if (isRefund) setStep('METHOD_SELECT'); else setStep('METHOD_SELECT'); }} className={`flex-[2] h-20 rounded-2xl font-bold uppercase text-zinc-950 transition-colors shadow-xl text-xl tracking-widest ${isRefund ? 'bg-red-500 hover:bg-red-400' : 'bg-lime-500 hover:bg-lime-400'}`}>{isZeroBalance ? 'Update Order' : isRefund ? 'Process Refund' : 'Continue to Payment'}</button>
        </div>
    </div>
  );
    ...
    <div className="h-full animate-in slide-in-from-right duration-300 flex flex-col items-center justify-center">
        <div className="w-full max-w-5xl grid grid-cols-2 gap-6 p-8">
            {['Cash', 'Clip', 'Transfer'].map((m) => (
                <button key={m} onClick={() => handleMethodSelect(m as PaymentMethod)} className="h-64 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 hover:border-lime-500 dark:hover:border-lime-400 border-2 border-zinc-200 dark:border-zinc-700 rounded-3xl flex flex-col items-center justify-center p-8 transition-all group shadow-lg">
                    {m === 'Cash' && <Banknote size={80} className="text-zinc-400 dark:text-zinc-500 group-hover:text-lime-600 dark:group-hover:text-lime-400 mb-6" />}
                    {m === 'Clip' && <CreditCard size={80} className="text-zinc-400 dark:text-zinc-500 group-hover:text-lime-600 dark:group-hover:text-lime-400 mb-6" />}
                    {m === 'Transfer' && <ArrowRightLeft size={80} className="text-zinc-400 dark:text-zinc-500 group-hover:text-lime-600 dark:group-hover:text-lime-400 mb-6" />}
                    <span className="text-3xl font-bold uppercase tracking-widest text-zinc-700 dark:text-zinc-300 group-hover:text-black dark:group-hover:text-white">{m}</span>
                </button>
            ))}
            {!isRefund && (
    ...
    <div className="grid grid-cols-2 gap-12 h-full p-8 animate-in slide-in-from-right duration-200">
        ...
        <div className="bg-zinc-50 dark:bg-zinc-800/30 rounded-3xl border border-zinc-200 dark:border-zinc-700/50 p-8 flex flex-col shadow-inner">
            <div className="flex justify-center mb-6 space-x-4">
                ...
                <button 
                    onClick={() => { setActiveInput('TIP'); setKeepChange(false); }} 
        ...
        <div className="flex flex-col justify-center">
            <div className="flex-1 flex flex-col justify-center items-center w-full max-w-md mx-auto space-y-6">
                ...
                <div className="w-full flex justify-between items-center border-b border-zinc-200 dark:border-zinc-800 pb-4">
                    ...
                    <span className="text-2xl font-mono font-bold text-zinc-700 dark:text-zinc-300">${Math.abs(balanceDue).toFixed(2)}</span>
                </div>
                ...
                >
                    ...
                    <span className="text-2xl font-mono font-bold text-lime-600 dark:text-lime-400">+${currentTip.toFixed(2)}</span>
                </div>

                <div className="bg-zinc-900 w-full p-6 rounded-3xl border border-zinc-800 shadow-xl text-center">
                    <span className="text-zinc-500 text-xs font-bold uppercase tracking-[0.2em] block mb-2">Total Charge</span>
                    <span className="text-6xl font-mono font-bold text-white tracking-tighter">${totalCharge.toFixed(2)}</span>
                </div>
                ...
                {selectedMethod === 'Cash' && !isRefund && (
                    <div className="w-full">
                        <div className="flex justify-between items-center bg-zinc-100 dark:bg-zinc-800 p-4 rounded-2xl">
                            <span className="text-sm font-bold text-zinc-500 uppercase tracking-widest">Change Due</span>
                            <span className={`font-mono text-3xl font-bold ${changeDue < 0 ? 'text-red-500' : 'text-zinc-900 dark:text-white'}`}>
                                ${changeDue.toFixed(2)}
                            </span>
                        </div>
            ...
            <div className="mt-8 w-full flex flex-col gap-4">
                <button 
                    ...
                    disabled={(selectedMethod === 'Cash' && changeDue < 0) || (selectedMethod === 'Clip' && tenderedValue.length === 0)}
                    className={`w-full h-24 rounded-2xl font-bold uppercase tracking-widest text-2xl transition-all shadow-xl hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center 
                        ${(selectedMethod === 'Cash' && changeDue < 0) || (selectedMethod === 'Clip' && tenderedValue.length === 0)
                            ? 'bg-zinc-200 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-600 cursor-not-allowed shadow-none' 
                            : isRefund ? 'bg-red-500 text-white hover:bg-red-400' : 'bg-lime-500 text-zinc-950 hover:bg-lime-400 shadow-lime-500/30'
                        }`}
                >
                    {selectedMethod === 'Clip' ? 'Process Card & Close' : 'Confirm Payment'}
                </button>
                <button onClick={() => setStep('METHOD_SELECT')} className="w-full h-16 rounded-xl bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 font-bold uppercase tracking-wider transition-colors">
                    Back to Methods
                </button>
            </div>
        </div>
    ...
    return (
      <div className="grid grid-cols-2 gap-12 h-full p-8 animate-in slide-in-from-right duration-200">
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-8 flex flex-col shadow-inner">
            <h3 className="text-zinc-400 font-bold uppercase tracking-widest mb-6 text-center text-sm">Add Payment Transaction</h3>
            
            <div className="relative mb-6">
                <DollarSign className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-500" size={32} />
                <input 
                    type="number" 
                    value={splitInput} 
                    readOnly 
            ...
            <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 flex flex-col mb-6 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                    {splitPayments.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-300 dark:text-zinc-700 opacity-50">
                            ...
                            <span className="text-xs uppercase font-bold tracking-widest">No Payments Added</span>
                        </div>
                    ) : (
                        splitPayments.map((p, idx) => (
                            <div key={idx} className="flex justify-between items-center p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800 animate-in slide-in-from-left-2">
                                <div className="flex items-center">
                                    ...
                                    <span className="font-bold text-zinc-700 dark:text-zinc-300 uppercase text-sm">{p.method}</span>
                                </div>
                                <div className="flex items-center">
                                    <span className="font-mono font-bold text-lg text-zinc-900 dark:text-white mr-3">${p.amount.toFixed(2)}</span>
                                    <button onClick={() => handleRemoveSplitPayment(idx)} className="text-zinc-400 hover:text-red-500 transition-colors">
                                        <X size={16} />
                                    </button>
                                </div>
                            </div>
                ...
                <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 space-y-2">
                    ...
                    <div className="flex justify-between items-center bg-zinc-100 dark:bg-zinc-800 p-3 rounded-xl">
                        ...
                        <span className={`font-mono text-2xl font-bold ${splitRemaining > 0.01 ? 'text-red-500' : 'text-lime-600 dark:text-lime-400'}`}>${splitRemaining.toFixed(2)}</span>
                    </div>
                </div>
            </div>

            <div className="flex gap-4 h-20">
                <button onClick={() => setStep('METHOD_SELECT')} className="w-1/3 bg-zinc-100 dark:bg-zinc-800 rounded-2xl font-bold uppercase text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">Cancel</button>
                <button 
      ...
      <div className="relative bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-[95%] h-[90vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden transition-colors duration-300">
         <div className="h-20 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-8 bg-zinc-50 dark:bg-zinc-950 shrink-0">
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white tracking-widest uppercase flex items-center">
                ...
                <span className="text-zinc-400 font-mono text-lg">#{Math.floor(Math.random() * 1000)}</span>
            </h2>
         ...
         {step === 'METHOD_SELECT' && (
             <div className="h-24 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex items-center justify-between px-8 shrink-0">
                <button onClick={() => setStep('ORDER_REVIEW')} className="text-sm font-bold text-zinc-500 hover:text-zinc-900 dark:hover:text-white underline uppercase tracking-wider">Back to Review</button>
                <div className="flex flex-col items-end">
                    <span className="text-xs uppercase text-zinc-500 tracking-[0.2em] font-bold mb-1">{isRefund ? 'Refund Due' : 'Balance Due'}</span>
                    <span className={`text-4xl font-mono font-bold ${isRefund ? 'text-red-500 dark:text-red-400' : 'text-zinc-900 dark:text-white'}`}>${Math.abs(balanceDue).toFixed(2)}</span>
                </div>
             </div>
         )}
      </div>
