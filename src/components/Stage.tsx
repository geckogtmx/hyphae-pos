...
import { Search, Utensils, Trash2, PencilLine, ShoppingBag, ChevronUp, ChevronDown, Crown, RotateCcw, Trophy, SlidersHorizontal } from 'lucide-react';
...
const Stage: React.FC<StageProps> = ({ activeConceptId = 'tacocracy', onLayoutChange: _onLayoutChange }) => {
    const { state, dispatch, total, tax, grandTotal } = useOrder();
    const { concepts, getProductsByConcept } = useMenuData();
    ...
    const handlePayment = (method: PaymentMethod, amountPaid: number, isFullPayment: boolean, confirmationNumber?: string, tenderedAmount?: number, tipAmount?: number) => {
        dispatch({
            type: 'CREATE_ORDER',
            payload: {
                method,
    ...
    const renderLoyaltyBanner = () => {
        ...
        const tier = LOYALTY_TIERS.find(t => t.id === profile.currentTierId);

        const nextTier = LOYALTY_TIERS
            .filter(t => t.minPunches > (tier?.minPunches || 0))
            .sort((a, b) => a.minPunches - b.minPunches)[0];
            ...
            <div className="w-full bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-3 flex items-center justify-between shrink-0 animate-in slide-in-from-top">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <div className="flex items-center space-x-2">
                            <span className={`text-sm font-bold uppercase tracking-wider text-${tier?.color || 'zinc'}-700 dark:text-${tier?.color || 'white'}`}>{tier?.name} Member</span>
                            <span className="text-zinc-400 dark:text-zinc-500 text-xs">|</span>
                            <span className="text-zinc-800 dark:text-zinc-300 font-bold text-xs">{profile.name}</span>
                        </div>
                ...
                {nextTier ? (
                    ...
                    <div className="flex items-center space-x-2 bg-yellow-100 dark:bg-yellow-900/20 px-3 py-1 rounded-full border border-yellow-200 dark:border-yellow-500/30">
                        ...
                        <span className="text-[10px] font-bold text-yellow-600 dark:text-yellow-400 uppercase">Top Tier Status</span>
                    </div>
                )}
            </div>
        );
    ...
    return (
        <div
            className="h-full w-full flex flex-col bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 overflow-hidden relative transition-colors duration-300"
        >
            {isCheckoutOpen && (
                <CheckoutModal
                                ...
                                {state.editingOrder ? (
                                    <div className="flex items-center space-x-3">
                                        <div className="flex items-center space-x-2 bg-orange-100 dark:bg-orange-500/20 px-3 py-1.5 rounded-lg border border-orange-200 dark:border-orange-500/50">
                                            ...
                                            <span className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wider">Editing Order #{state.editingOrder.id}</span>
                                        </div>
                                        <button onClick={handleCancelEdit} className="flex items-center space-x-1 text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800 px-2 py-1 rounded transition-colors">
                                            <RotateCcw size={14} />
                                    ...
                                    <div className="relative group">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-lime-500 dark:group-focus-within:text-lime-400 transition-colors" size={16} />
                                        <input
                                            type="text"
                                        ...
                                        >
                                            <div className="w-full">
                                                <div className="flex justify-between items-start">
                                                    <span className={`font-bold text-left text-sm md:text-base xl:text-lg leading-tight block mb-1 group-hover:text-${activeConcept?.color.split('-')[0]}-600 dark:group-hover:text-${activeConcept?.color.split('-')[0]}-400 transition-colors pr-6 text-zinc-900 dark:text-zinc-100`}>
                                                        {item.name}
                                                    </span>
                                                </div>
                                                {item.requiresMods && (
                                                    <span className="font-mono text-[9px] text-zinc-500 uppercase tracking-widest bg-zinc-100 dark:bg-zinc-800 px-1 rounded-md inline-block mt-1">Mods Req</span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex justify-end items-end w-full border-t border-zinc-100 dark:border-zinc-800 pt-2 mt-2 group-hover:border-zinc-200 dark:group-hover:border-zinc-700">
                                                <span className="font-mono text-base md:text-lg xl:text-xl font-bold text-zinc-700 dark:text-zinc-200">${item.price.toFixed(2)}</span>
                                            </div>

                                            {hasModifiers && (
                                                <div className="absolute top-3 right-3 text-zinc-400 group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors bg-zinc-100 dark:bg-zinc-900/50 rounded-md p-0.5" title="Modifiers Available">
                                                    <SlidersHorizontal size={14} />
                                                </div>
                                            )}

                                            {!hasModifiers && (
                                                <div className={`absolute top-0 right-0 w-2 h-2 border-t-2 border-r-2 border-zinc-200 dark:border-zinc-800 transition-colors group-hover:border-${activeConcept?.color.split('-')[0]}-400 rounded-tr-xl`}></div>
                                            )}
                                        </button>
                                    );
                                })}
      ...
      `}>
                        ...
                        {mode === 'BUILD' && buildingProduct && (
                            <div className="w-full h-16 shrink-0 bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900/50 border border-dashed border-lime-500/50 dark:border-lime-400/50 rounded-xl p-3 flex flex-row items-center justify-between animate-pulse">
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-bold text-zinc-900 dark:text-zinc-300 text-sm truncate">{buildingProduct.name}</span>
                                </div>
                            </div>
                        )}
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-4 flex-1">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center">
                                                    <span className="font-bold text-sm text-zinc-900 dark:text-zinc-200 block truncate">{item.name}</span>
                                                    {item.isDiscounted && <span className="ml-2 text-[9px] bg-yellow-400 text-zinc-900 px-1 rounded font-bold uppercase">Gold Perk</span>}
                                                </div>
                                                ...
                                                <div className="mt-1 space-y-1">
                                                    <div className="flex flex-wrap gap-1">
                                                        {item.selectedModifiers?.filter(m => {
                                                            ...
                                                            <span key={`${mod.groupId}-${mod.optionId}`} className="text-[9px] px-1 bg-zinc-100 dark:bg-zinc-800 rounded-md text-zinc-500 dark:text-zinc-400 truncate max-w-[150px] inline-flex items-center">
                                                                {mod.variation !== 'Normal' && <span className={`mr-1 ${mod.variation === 'No' ? 'text-red-500 dark:text-red-400' : 'text-blue-500 dark:text-blue-400'}`}>{mod.variation}</span>}{mod.name}
                                                            </span>
                                                        ))}
                                                    </div>

                                                    {item.selectedModifiers?.filter(m => item.modifierGroups?.find(g => g.id === m.groupId)?.variant === 'sub_item').map(subItem => (
                                                        <div key={subItem.optionId} className="flex flex-row flex-wrap items-center gap-2 mt-1">
                                                            ...
                                                            <div className="flex flex-wrap gap-1">
                                                                {item.selectedModifiers?.filter(m => {
                                                                    ...
                                                                    <span key={nestedMod.optionId} className="text-[9px] px-1 bg-zinc-100 dark:bg-zinc-800/50 text-zinc-500 rounded border border-zinc-200 dark:border-zinc-800">
                                                                        {nestedMod.name}
                                                                    </span>
                                                                ))}
                                                            </div>
                                            ...
                                            <div className="flex flex-col items-end">
                                                <span className={`font-mono font-bold text-lg ${item.isDiscounted ? 'text-yellow-500 dark:text-yellow-400 line-through decoration-zinc-500 text-sm' : 'text-zinc-700 dark:text-zinc-200'}`}>
                                                    ${(item.price + (item.selectedModifiers?.reduce((a, m) => a + m.price, 0) || 0)).toFixed(2)}
                                                </span>
                                                {item.isDiscounted && <span className="font-mono font-bold text-lg text-lime-600 dark:text-lime-400">$0.00</span>}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-2 pl-2 self-center">
                                            ...
                                            <button
                                                onClick={(e) => { e.stopPropagation(); dispatch({ type: 'REMOVE_ITEM', payload: item.uniqueId }); }}
                ...
                <div className="h-14 shrink-0 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex items-center px-4 md:px-6 shadow-inner z-20 overflow-hidden">
                    ...
                    <div className="flex-[1.5] flex items-center justify-between min-w-0">
                        <span className="font-bold text-xs md:text-sm xl:text-base text-zinc-400 uppercase tracking-wide truncate mr-2">Total Due</span>
                        <span className="font-mono text-lg md:text-2xl xl:text-3xl font-bold text-lime-600 dark:text-lime-400 truncate">${grandTotal.toFixed(2)}</span>
                    </div>
                </div>
