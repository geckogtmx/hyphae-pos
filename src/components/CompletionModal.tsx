            ...
            <div className="bg-zinc-900 border border-zinc-800 w-full max-w-4xl h-[600px] rounded-3xl shadow-2xl flex overflow-hidden">
                <div className="w-1/3 bg-zinc-950 p-6 flex flex-col border-r border-zinc-800">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-6">
                            ...
                            <div className="p-3 bg-zinc-900 border border-zinc-800 rounded-xl">
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                    {Object.entries(packagingInventory).map(([sku, count]) => (
                                        <div key={sku} className="flex justify-between items-center text-xs border-b border-zinc-800 last:border-0 pb-1 last:pb-0">
                                            <span className="text-zinc-400 text-[10px]">{sku.replace('SKU_', '')}</span>
                                            <span className="font-mono font-bold text-white bg-zinc-800 px-1.5 rounded">x{count}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="space-y-4 pt-4 border-t border-zinc-800">
                            <div>
                                <span className="text-[10px] font-bold uppercase text-zinc-600 block mb-1">Queue Time</span>
                                <div className="text-xl font-mono text-zinc-400">{formatSecs(getDurationSecs(order.createdAt, order.cookingStartedAt))}</div>
                            </div>
                            <div>
                                <span className="text-[10px] font-bold uppercase text-zinc-600 block mb-1">Cook Time</span>
                                <div className="text-xl font-mono text-orange-500">{formatSecs(getDurationSecs(order.cookingStartedAt, order.readyAt))}</div>
                            </div>
                            <div>
                                <span className="text-[10px] font-bold uppercase text-zinc-600 block mb-1">Pack/Wait Time</span>
                                <div className="text-xl font-mono text-lime-500">{formatSecs(getDurationSecs(order.readyAt))}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 p-8 flex flex-col bg-zinc-900">
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            <h3 className="text-lg font-bold text-zinc-300 uppercase tracking-widest">Add Tip?</h3>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center max-w-sm mx-auto w-full">
                        ...
                        <div className="grid grid-cols-3 gap-3 w-full mb-6">
                            ...
                            <button onClick={handleTipBackspace} className="h-16 bg-zinc-800 rounded-xl flex items-center justify-center text-zinc-400 hover:text-red-400 hover:bg-zinc-700 active:bg-red-500 active:text-white transition-colors">
                                <Delete size={24} />
                            </button>
                        </div>
