...
import { User, Crown, ArrowLeft, ArrowRight, CheckCircle, XCircle, Eraser, Delete, Activity, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
import { LOYALTY_PROFILES, LOYALTY_TIERS, LOYALTY_TRANSACTIONS, LOYALTY_CARDS } from '../data/mock_data';
import { LoyaltyProfile } from '../types';

interface LoyaltyScreenProps {
    onGuestAccess: () => void;
    onLoginSuccess: (cardNumber: string) => void;
    isAddMode?: boolean; // New prop to indicate we are adding to an existing order
}

const LoyaltyScreen: React.FC<LoyaltyScreenProps> = ({ onGuestAccess, onLoginSuccess, isAddMode = false }) => {
    ...
    const handleLoyaltySubmit = () => {
        // New Relational Lookup Logic
        const cardCode = loyaltyInput.toUpperCase();
        
        // 1. Find the Card
        const card = LOYALTY_CARDS.find(c => c.code === cardCode && c.status === 'ACTIVE');
        
        if (card) {
            // 2. Find the User
            const foundProfile = LOYALTY_PROFILES.find(p => p.id === card.userId);
            
            if (foundProfile) {
                // Simulate fetching recent transactions for preview
                const recentTx = LOYALTY_TRANSACTIONS
                    .filter(t => t.customerId === foundProfile.id)
                    .sort((a,b) => b.timestamp - a.timestamp)
                    .slice(0, 3); // Just top 3 for preview

                setPreviewProfile({ ...foundProfile, recentTransactions: recentTx, activeCard: card });
            ...
            {!showLoyaltyInput ? (
                <div className="flex gap-8 w-full max-w-4xl h-96">
                    ...
                    >
                        ...
                        <span className="text-4xl font-bold tracking-widest text-zinc-400 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-white mb-4">GUEST</span>
                        <span className="text-zinc-500 text-sm font-mono uppercase tracking-wider">Quick Order</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="text-4xl font-bold tracking-widest text-zinc-400 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-white mb-4">CARD</span>
                        <span className="text-zinc-500 text-sm font-mono uppercase tracking-wider">Loyalty Member</span>
                    </button>
                </div>
            ) : (
                <div className="flex flex-col items-center w-full max-w-md animate-in zoom-in-95 duration-200 h-full justify-center">
                    {lookupStatus === 'FOUND' && previewProfile ? (
                        <div className="w-full bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900 rounded-3xl p-6 border-2 border-lime-500/50 shadow-2xl flex flex-col items-center animate-in slide-in-from-bottom duration-300">
                            ...
                            <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-1">Welcome Back</h3>
                            <h2 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2 text-center">{previewProfile.name}</h2>
                            <div className="flex items-center gap-2 mb-6">
                                <span className="px-3 py-1 bg-yellow-400 text-zinc-900 font-bold rounded-full text-xs uppercase tracking-wider shadow-sm flex items-center">
                                    <Crown size={12} className="mr-1 fill-current" /> {LOYALTY_TIERS.find(t=>t.id===previewProfile.currentTierId)?.name || 'Member'}
                                </span>
                                <span className="text-zinc-400 text-xs">|</span>
                                <span className="text-zinc-500 font-mono text-xs">{previewProfile.currentPoints.toFixed(0)} PTS</span>
                            </div>

                            {/* TRANSACTION PREVIEW */}
                            {previewProfile.recentTransactions && previewProfile.recentTransactions.length > 0 && (
                                    ...
                                    <div className="space-y-2">
                                        {previewProfile.recentTransactions.map(tx => (
                                            <div key={tx.id} className="flex justify-between items-center text-xs">
                                                ...
                                                <span className={`font-mono font-bold ${tx.points > 0 ? 'text-lime-600 dark:text-lime-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                                    {tx.points > 0 ? '+' : ''}{tx.points.toFixed(0)}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <>
                            <div className="w-full flex items-center gap-4 mb-6">
                                    ...
                                    <div className={`flex-1 h-14 bg-zinc-100 dark:bg-zinc-950 border-b-2 flex items-center justify-center relative overflow-hidden transition-colors ${lookupStatus === 'ERROR' ? 'border-red-500 bg-red-50 dark:bg-red-900/10 animate-[shake_0.5s_ease-in-out]' : 'border-lime-500 dark:border-lime-400/50'}`}>
                                        {lookupStatus === 'ERROR' ? (
                                            <span className="font-bold text-red-500 dark:text-red-400 uppercase tracking-widest text-sm flex items-center"><XCircle size={16} className="mr-2"/> CARD NOT FOUND</span>
                                        ) : (
                                            <span className="font-mono text-3xl tracking-[0.2em] text-lime-600 dark:text-lime-400 font-bold uppercase">
                                                {loyaltyInput || <span className="text-zinc-400 dark:text-zinc-800 text-lg tracking-normal opacity-50 font-sans uppercase">Enter Card ID</span>}
                                            </span>
                                        )}
                                    </div>
                            </div>
                            ...
                            <div className="w-full bg-zinc-50 dark:bg-zinc-900 dark:bg-zinc-900/50 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800 shadow-2xl">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <button onClick={handleLoyaltyClear} className="bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white rounded-xl flex flex-col items-center justify-center py-3 transition-all border border-zinc-200 dark:border-zinc-800">
                                        <Eraser size={20} className="mb-1" />
                                        <span className="text-[10px] uppercase font-bold tracking-wider">Clear</span>
                                    </button>
                                    <button onClick={handleLoyaltyBackspace} className="bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white rounded-xl flex flex-col items-center justify-center py-3 transition-all border border-zinc-200 dark:border-zinc-800">
                                        <Delete size={20} className="mb-1" />
                                        <span className="text-[10px] uppercase font-bold tracking-wider">Back</span>
                                    </button>
                                    <button onClick={handleLoyaltySubmit} disabled={loyaltyInput.length === 0} className={`bg-lime-500 active:scale-[0.98] rounded-xl text-zinc-950 font-bold uppercase tracking-widest shadow-lg transition-all flex flex-col items-center justify-center py-3 border border-transparent ${loyaltyInput.length === 0 ? 'opacity-50 cursor-not-allowed bg-zinc-200 dark:bg-zinc-700 text-zinc-400 dark:text-zinc-500' : 'hover:bg-lime-400'}`}>
