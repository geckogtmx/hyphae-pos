...
import { Product, ModifierGroup, SelectedModifier, OrderItem } from '../types';
...
const OrderBuilder: React.FC<OrderBuilderProps> = ({ product, editItem, onComplete, onCancel, onStepChange }) => {
  const [currentGroupIndex, setCurrentGroupIndex] = useState(0);
  const [selectedModifiers, setSelectedModifiers] = useState<SelectedModifier[]>(editItem?.selectedModifiers || []);
  ...
  const isGroupVisible = (group: ModifierGroup) => {
  ...
  const getStepHeader = () => {
      ...
      if (currentGroup.dependency) {
          const parentMod = selectedModifiers.find(m => m.groupId === currentGroup.dependency?.groupId);
          if (parentMod) {
  ...
  }, [autoAdvanceTrigger, groups.length, currentGroupIndex]);

  const handleModifierToggle = (group: ModifierGroup, optionId: string, name: string, price: number) => {
    const existingIndex = selectedModifiers.findIndex(m => m.groupId === group.id && m.optionId === optionId);
  ...
  const updateVariation = (groupId: string, optionId: string, variation: any) => {
        ...
        <div className="flex items-center">
            ...
            <div>
                ...
                <div className="flex items-center space-x-3 text-xs font-mono mt-0.5">
                    <span className="text-zinc-500 dark:text-zinc-400">STEP {currentGroupIndex + 1}/{groups.length}:</span>
                    <span className="text-lime-600 dark:text-lime-400 uppercase tracking-widest font-bold">{getStepHeader()}</span>
                    {currentGroup.required ? (
                         <span className="px-1.5 py-0.5 rounded bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/50 text-[9px] uppercase font-bold tracking-wider">Required</span>
                    ) : (
                         <span className="px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 text-[9px] uppercase font-bold tracking-wider">Optional</span>
                    )}
                </div>
            </div>
        </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-zinc-50 dark:bg-zinc-950">
         <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {currentGroup.options.map(option => {
                const isSelected = selectedModifiers.some(m => m.groupId === currentGroup.id && m.optionId === option.id);
                const selectedMod = selectedModifiers.find(m => m.groupId === currentGroup.id && m.optionId === option.id);
                ...
                return (
                    <div key={option.id} className="h-32">
                        <ActionGridButton 
                            ...
                            onClick={() => handleModifierToggle(currentGroup, option.id, option.name, option.price)}
                            onVariationChange={isSelected ? (v) => updateVariation(currentGroup.id, option.id, v) : undefined}
                        />
                    </div>
                );
            })}
