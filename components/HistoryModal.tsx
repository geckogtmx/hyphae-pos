...
const HistoryModal: React.FC<HistoryModalProps> = ({ onClose }) => {
  ...
  const historyOrders = [...state.completedOrders, ...legacyCompleted]
  ...
  const renderOrderDetails = (order: SavedOrder) => (
    <div className="bg-black/20 rounded-lg p-3 space-y-3 mb-3 font-mono">
        {order.items.map((item, idx) => (
            <div key={`${item.uniqueId}-${idx}`} className="border-b border-zinc-800/50 last:border-0 pb-3 last:pb-0">
                <div className="flex justify-between items-start">
                    <span className="text-white font-bold text-sm uppercase leading-tight">{item.name}</span>
                ...
                <div className="mt-1 space-y-1 pl-2">
                        ...
                        return true;
                         ...
                         <div key={i} className="text-xs text-zinc-400 flex items-start">
                             {mod.variation !== 'Normal' ? (
                                 <span className={`font-bold mr-1.5 px-1 rounded text-[10px] uppercase
                                     ${mod.variation === 'No' ? 'bg-red-900/50 text-red-400' : 
                                       mod.variation === 'Side' ? 'bg-blue-900/50 text-blue-400' : 
                     ...
                     {item.selectedModifiers?.filter(m => item.modifierGroups?.find(g => g.id === m.groupId)?.variant === 'sub_item').map((subItem, i) => (
      ...
      <div className="relative bg-zinc-900 border border-zinc-800 w-full max-w-2xl h-[80vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-950 shrink-0">
            <div className="flex items-center space-x-3">
                ...
                <div>
                    <h2 className="text-lg font-bold text-white tracking-widest uppercase">Order History</h2>
                ...
                >
                    ...
                    >
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="text-left">
                                <div className="flex items-center space-x-2">
                                    ...
                                    {order.isLoyalty && <Crown size={14} className="text-yellow-500 fill-yellow-500/20" />}
                                ...
                                <div className="flex items-center space-x-3 text-xs text-zinc-500 mt-1 font-mono">
                                    <span className="flex items-center">
                                        ...
                                        {order.completedAt ? new Date(order.completedAt).toLocaleTimeString() : 'Unknown'}
                                    </span>
                                    <span className="flex items-center">
                                        {order.paymentStatus === 'Paid' ? <DollarSign size={12} className="mr-1" /> : null}
                                        {order.paymentStatus}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="text-right">
                            <div className="font-mono font-bold text-white text-lg">${order.total.toFixed(2)}</div>
                            {order.tipAmount ? (
                                <span className="text-xs text-lime-400 font-mono">+${order.tipAmount.toFixed(2)} Tip</span>
                            ) : (
                    ...
                    {expandedId === order.id && (
                        <div className="px-4 pb-4 animate-in slide-in-from-top-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <h4 className="text-xs font-bold text-zinc-500 uppercase mb-2 tracking-widest">Order Items</h4>
                                    ...
                                    <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800 flex justify-between items-center">
                                        <div className="flex items-center gap-4 text-xs text-zinc-400">
                                            <div className="flex items-center gap-1" title="Staff ID">
                                                <User size={12} /> <span className="font-mono">{order.systemInfo?.staffId || 'UNK'}</span>
                                            </div>
                                            <div className="flex items-center gap-1" title="Terminal ID">
                                                <Monitor size={12} /> <span className="font-mono">{order.systemInfo?.terminalId || 'UNK'}</span>
                                            </div>
                                            <div className="flex items-center gap-1" title="Store ID">
                                                <MapPin size={12} /> <span className="font-mono">{order.systemInfo?.storeId || 'UNK'}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                                        <h4 className="text-xs font-bold text-zinc-500 uppercase mb-3 tracking-widest">Summary</h4>
                                    ...
                                    <div className="flex gap-2">
                                        <div className="flex-1 bg-zinc-950 p-3 rounded-lg border border-zinc-800 flex items-center justify-center flex-col">
                                            ...
                                            <div className="flex items-center text-zinc-300 font-bold">
                                                 {order.confirmationNumber ? <CreditCard size={16} className="mr-2"/> : <Banknote size={16} className="mr-2"/>}
                                                 <span>Payment</span> 
                                            </div>
                                        </div>
                                        {order.confirmationNumber && (
                                             <div className="flex-1 bg-zinc-950 p-3 rounded-lg border border-zinc-800 flex items-center justify-center flex-col">
                                                ...
                                                <span className="font-mono text-zinc-300 font-bold">{order.confirmationNumber}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
